...
Research code for point-in-time macro regime inference and causal cross-asset
allocation. The repository contains two published model lines:

- **Model 01** is the retained monthly benchmark and is frozen.
- **Model 02** is the promoted weekly model. Its current iteration is closed
  for now, but the model is **not frozen** and may be revised in a later,
  explicitly versioned update.
